    ...
    @foreach ($datas as $value)
        ...
        <div class="projects">
            ...
            @if ($value?->subProjects?->name)
                <div class="subpriject"><strong>Sub-Project:</strong> {{ blank_if_null($value?->subProjects?->name) }}
                </div>
            @endif
            ...
            <tbody>
                @if (isset($value->activities) && count($value->activities))
                    @foreach ($value->activities as $key => $activity)
                        @php
                            $usageArray = isset($usage) && $usage instanceof \Illuminate\Http\JsonResponse ? $usage->toArray() : (isset($usage) ? $usage : []);
                            $usage = is_array($usageArray)
                                ? $usageArray
                                : [
                                    'originalQty' => 0,
                                    'remainingQty' => 0,
                            ...
                            $percent =
                                isset($usage['originalQty']) && $usage['originalQty'] != 0
                                    ? number_format(($activity->qty / $usage['originalQty']) * 100, 2)
                                    : ' ';
                            $cummQty = isset($usage['originalQty']) && isset($usage['remainingQty']) ? abs($usage['originalQty'] - $usage['remainingQty']) : 0;
                        @endphp
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            ...
                            <td>{{ isset($usage['originalQty']) ? $usage['originalQty'] : ' ' }}</td>
                            <td>{{ isset($usage['remainingQty']) ? $usage['remainingQty'] : ' ' }}</td>
        ...
        <table>
            ...
            <tbody>
                ...
                @else
                    <tr>
                        <td colspan="10" style="text-align: center;">No assets used.</td>
                    </tr>
                @endif
            </tbody>
        </table>
        ...
        <table>
            ...
            <tbody>
                @if (isset($value->material))
                    @foreach ($value->material as $key => $materialData)
                        <tr>
                            ...
                            <td class="td-line-break">{{ blank_if_null($materialData?->materials?->specification) }}
                            </td>
                ...
                @else
                    <tr>
                        <td colspan="10" style="text-align: center;">No materials used.</td>
                    </tr>
                @endif
            </tbody>
        </table>
        ...
        <table>
            ...
            <tbody>
                ...
                @else
                    <tr>
                        <td colspan="10" style="text-align: center;">No labour data.</td>
                    </tr>
                @endif
            </tbody>
        </table>
        ...
        <div class="images">
            @if (isset($value->safetie))
                @foreach ($value->safetie as $safety)
                    @if ($safety->img)
                        <img src="{{ url('/upload/' . $safety->img) }}" alt="Safety Image" width="200"
                            height="200">
                    @endif
                @endforeach
            @endif
        ...
        <div class="images">
            @if (isset($value->historie))
                @foreach ($value->historie as $hinderance)
                    @if ($hinderance->img)
                        <img src="{{ url('/upload/' . $hinderance->img) }}" alt="Hinderance Image" width="200"
                            height="200">
                    @endif
                @endforeach
            @endif
        ...
        <div class="images">
            @if (isset($value->activities))
                @foreach ($value->activities as $activity)
                    @if ($activity->img)
                        <img src="{{ url('/upload/' . $activity->img) }}" alt="Activity Image" width="200"
                            height="200">
                    @endif
                @endforeach
            @endif
