...
@push('scripts')
    ...
    <script>
        ...
        function workprocessSubprojectsDropdown(projectId) {
            ...
            $.get(baseUrl + 'company/activities/subprojects/' + projectId, function(data) {
                ...
                $('#from_subproject_work_process').append('<option value="">----Select Subproject----</option>');
        ...
        function updateSubprojectsDropdownStocks(projectId) {
            $.get(baseUrl + 'company/activities/storeprojects/' + projectId, function(data) {
                ...
                $.each(data, function(key, value) {
                    console.log(value);
                    $('#from_subproject_stocks').append('<option value="">----Select Store----</option>');
        ...
        function updateSubprojectsDropdown(projectId) {
            $.get(baseUrl + 'company/activities/subprojects/' + projectId, function(data) {
                ...
                $('#from_subproject').append('<option value="">----Select SubProject----</option>');
                $.each(data, function(key, value) {
                    $.each(value.sub_project, function(subkey, subvalue) {
        ...
        function renderWorkOverview(response, project, subproject, date) {
            ...
            $('#totalLabourTotal').text(response.totalLabourTotal);
            console.log("------------------------------------", response.totalLabourTotal);
        ...
        function renderVendorLabourList(vendorData) {
            ...
            if (vendorData && vendorData.length > 0) {
                ...
                vendorData.forEach(function(vendorItem) {
                    ...
                    var labourTotal = vendorItem.labour_total;
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    switch (filterName) {
                        case 'inprogress':
                            console.log("------------------------>>>>");
                            console.log(response.inProgressactivites);
                            ("------------------------<<<<<<<<<<<");
                            inprogresspopulateTable(response.inProgressactivites)
                            break;
                        case 'completed':
                            console.log("------------------------>>>>");
                            console.log(response.completedactivites);
                            ("------------------------<<<<<<<<<<<");
                            completedpopulateTable(response.completedactivites)
                            break;
                        case 'notstart':
                            console.log("------------------------>>>>");
                            console.log(response.notStartactivites);
                            ("------------------------<<<<<<<<<<<");
                            notStartpopulateTable(response.notStartactivites)
                            break;
                        case 'delay':
                            console.log("------------------------>>>>");
                            console.log(response.delayactivites);
                            ("------------------------<<<<<<<<<<<");
        ...
        function inprogresspopulateTable(data) {
            ...
            dprContainer.empty();
            let tableHtml = `
                <table class="table table-bordered dataTable" id="inprogressActiveTable">
                    <thead>
                    <tr>
            ...
            if (data && data.length > 0) {
                data.forEach((activity, index) => {
                    // Total completed quantity = sum of qty from history
                    const completedQty = activity.activities_history.reduce((sum, h) => sum + Number(h.qty || 0),
                        0);

                    const plannedQty = Number(activity.qty || 0);
                    const rate = Number(activity.rate || 0);
                    const estAmount = plannedQty * rate;

                    // Excess = actual - planned (can be negative)
                    const excessQty = Math.abs(completedQty - plannedQty);
                    const excessAmount = Math.abs(excessQty * rate);

                    // % Completion (capped at 100% if needed?)
                    const percentageCompletion = plannedQty > 0 ? ((completedQty / plannedQty) * 100).toFixed(2) :
                        0;
                    ...
                    tableHtml += `
                        <tr>
                            ...
                            <td>${estAmount.toFixed(2)}</td>
                            <td>${completedQty}</td>
                            <td>${(completedQty * rate).toFixed(2)}</td>
                            <td>${percentageCompletion}%</td>
                            <td>${excessQty}</td>
                            <td>${excessAmount.toFixed(2)}</td>
                        </tr>
                        `;
                });
            } else {
                tableHtml += '<tr><td colspan="11">No data available</td></tr>';
            }
        ...
        // Listen for changes on the stocks filter form and update data accordingly
        $('#filter-form-stocks').on('change', function() {
            loadInventoryStocks();
        });

        // Also allow clicking on filter-inventory-stock-details buttons to trigger data load
        $('.filter-inventory-stock-details').on('click', function(event) {
            event.preventDefault();
            const filterName = $(event.target).data('name');
            loadInventoryStocks(filterName);
        });
        ...
        // Main function to load inventory stocks, optionally with a filterName override
        function loadInventoryStocks(filterNameOverride = null) {
            const project = $('#from_project_stocks').val();
            const subproject = $('#from_subproject_stocks').val();
            const date = $('#date_stocks').val();
            let filterName = filterNameOverride;

            // If not provided, get from checked radio or default to 'material'
            if (!filterName) {
                filterName = $('input[name="filterName"]:checked').data('name') || 'material';
            }
        ...
        function materialstacks(data) {
            ...
            <tr>
                ...
                <td>${stock.materials.specification!==null && stock.materials.specification!=="NULL" ? stock.materials.specification : ''}</td>
        ...
        function machinestocks(data) {
            ...
            <tr>
                ...
                <td>${stock.assets.specification!==null && stock.assets.specification!=="NULL" ? stock.assets.specification : ''}</td>
