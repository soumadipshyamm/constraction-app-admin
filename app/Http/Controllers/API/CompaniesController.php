...
class CompaniesController extends BaseController
{
    public function companiesList()
    {
        ...
        $data = Companies::where('company_id', $authCompany)->get();
        if (count($data) > 0) {
    ...
    {
        $authCompany = Auth::guard('company-api')->user()->company_id;
        $data = Companies::where('id', $uuid)->where('company_id', $authCompany)->get();
        $message = $data->isNotEmpty() ? 'Fetch Companies List Successfully' : 'Companies List Data Not Found';
